...
function CheckOut() {
  ...
  const paymentClick=(aaa)=>{
    switch (aaa) {
    case "credit_cart":
        setSelPayment({tit:"신용카드",text:'"결제 승인"을 클릭하시면 KCP사이트로 이동 후 결제를 완료하실 수 있습니다.',color:"black"});
        setSelPay('credit_cart');
    break;
    case "kakaopay":
        setSelPayment({tit:"카카오페이",text:'"결제 승인"을 클릭하시면 결제를 완료할 수 있는 카카오페이 웹사이트로 이동될 것입니다.',color:"black"});
        setSelPay('kakaopay');
    break;
    case "naverpay":
        setSelPayment({tit:"네이버페이",text:'"결제 승인"을 클릭하시면 결제를 완료할 수 있는 네이버페이 웹사이트로 이동될 것입니다.',color:"black"});
        setSelPay('naverpay');
    break;                    
    default: 
        setSelPayment({text:'올바른 결제 방법을 선택해주세요.',color:"red"});
        setSelPay('');
    break;
    }
  };
          ...
          <div className="check-out-box">
            ...
            <div className="payment-outbox">
                <div className="payment">
                  <div className={`payment-imgbox ${selPay === 'credit_cart' ? 'on' : ''}`} onClick={() => paymentClick('credit_cart')}>
                    <img
                      src={process.env.PUBLIC_URL + "/images/icons/credit_card.svg"}
                      alt="credit_card"
                    />
                  </div>
                  <div className={`payment-imgbox ${selPay === 'kakaopay' ? 'on' : ''}`} onClick={() => paymentClick('kakaopay')}>
                    <img
                      src={process.env.PUBLIC_URL + "/images/icons/kakaopay.svg"}
                      alt="kakaopay"
                    />
                  </div>
                  <div className={`payment-imgbox ${selPay === 'naverpay' ? 'on' : ''}`} onClick={() => paymentClick('naverpay')}>
                    <img
                      src={process.env.PUBLIC_URL + "/images/icons/naverpay.svg"}
                      alt="naverpay"
                    />
                  </div>
