...
function MyPageModifyEml({ setShowCart }) {
  ...
  const saveModify = (e) => {
    if (totalValid()) {
      ...
      let currEml = JSON.parse(sessionStorage.getItem("minfo")).eml;
      console.log(currid);
      memData.some((v) => {
        if (v.uid == currid) {
          if (currEml !== undefined && currEml !== "") {
            //만약에 세션에있는 이메일이 undefined가 아니면서 빈스트링이 아니면
            v.eml = userId; //입력한값을 v.eml에 넣기
          } else {
            v.uid = userId; //userId v.uid에 업데이트
          }
