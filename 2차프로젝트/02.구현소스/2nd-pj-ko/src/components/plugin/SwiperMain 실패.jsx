...
export default function SwiperMain() {
  ...
  useEffect(() => {
    // Set timeout for the zoom-in effect
    const timer = setTimeout(() => {
      setZoomIn(true);
    }, 1000); // Adjust timing for zoom-in effect
  ...
  return (
    <div className={`swiper-h-container ${zoomIn ? "zoomed-in" : ""}`}>
      {/* 가로방향 스와이프 설정///////////////////// */}
      <Swiper
        className={`mySwiper ${zoomIn ? "zoomed-in" : ""}`}
        spaceBetween={0}
        pagination={{
          clickable: true,
        }}
        navigation={true}
        initialSlide={1} // Start from the "man" page
